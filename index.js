const solver = (sudoku) => {//Set block
    ...
    const checkSurroundings = (col, row, num, map) => {
        ...
        */
        ...
        for (let i = rowEnd - 3; i < rowEnd; i++) { //loop through each row
            for (let j = colEnd - 3; j < colEnd; j++) { //loop through each column
    ...
    for (let row = 0; row < 9; row++) { //loop through row
        for (let col = 0; col < 9; col++) { //loop through column
            ...
            for (let i = 1; i < 10; i++) {  //checking numbers 1-9 one at a time in current position
                ...
                if (canBePlaced && numberBelongs) { //if number can be placed here and nowhere else
...
let solvedSudoku = solver(sudoku);
